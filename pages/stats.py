...
with col2:
    ...
    team_b = st.selectbox(
        ...
        remaining,
        index=(
            remaining.index("Chennai Super Kings")
            if "Chennai Super Kings" in remaining
            else 0
        ),
        key="h2h_b",
    )
...
# Calculate real wins from actual data
h2h_temp = matches[
    ((matches["team1"] == team_a) & (matches["team2"] == team_b))
    | ((matches["team1"] == team_b) & (matches["team2"] == team_a))
]
team_a_wins = (h2h_temp["winner"] == team_a).sum()
team_b_wins = (h2h_temp["winner"] == team_b).sum()

# Pie Chart
fig = go.Figure(
    data=[go.Pie(labels=[f"{team_a} Wins", f"{team_b} Wins"], values=[team_a_wins, team_b_wins], hole=0.5)]
)

fig.update_layout(
    paper_bgcolor="#0E1117", font_color="white", margin=dict(l=20, r=20, t=20, b=20)
)
...
if h2h.empty:
    ...
    recent = (
        h2h[["Season", "date", "venue", "winner", "win_by_runs", "win_by_wickets"]]
        .tail(5)
        .iloc[::-1]
    )
    recent.columns = [
...
st.markdown(
    '<div style="height:1px; background:linear-gradient(90deg, transparent, rgba(212,175,55,0.15), transparent); margin:40px 0;"></div>',
    unsafe_allow_html=True,
)
