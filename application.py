...
with col_cm:
        # Keep the content inside the string flat, but notice how the Python variable block is indented!
        matrix_html = (
        ...
        for idx, score in enumerate(metrics['cv_scores']):
            with cv_cols[idx]:
                cv_html = (
                    ...
                    f'<div style="font-size:9px; letter-spacing:1px; text-transform:uppercase; color:rgba(220,210,185,0.35); margin-bottom:4px;">Fold {idx+1}</div>\n'
                    f'<div style="font-family:\'DM Mono\',monospace; font-size:15px; color:#e8d89a; font-weight:500;">{score:.2%}</div>\n'
                    '</div>'
                )
